...
void M_Gumba::m_init(Vec2 Pos, Layer * layer, Vector<Block*> *v, Vector<M_Gumba*> *mv)
{

		mush = Sprite::create("gumba.png");
...
void M_Gumba::m_update(float dt, float MapPosX)
{
...
bool M_Gumba::BottomCollisionCheck(Vec2 HPos)
{
	for (Block * bl : *bvv)
	{
...
bool M_Gumba::LeftMoveCollisionCheck(Vec2 HPos, float MapPosX)
{
	HPos.x -= MoveSpeed * movePower;
...
bool M_Gumba::RightMoveCollisionCheck(Vec2 HPos, float MapPosX)
{
	HPos.x += MoveSpeed * movePower;
	for (Block * bl : *bvv)
	{
...
void M_Gumba::removethis()
{
	
	mushv->eraseObject(this);
}

Rect M_Gumba::m_getRect()
{
	return mush->boundingBox();
}

void M_Gumba::m_setVisivle()
{
...
void M_Gumba::m_dead()
{
	if (burn == true)
	{
		...
		Sprite * eff = Sprite::create("gumba.png");
		...
		//auto call = CallFunc::create(CC_CALLBACK_0(M_Gumba::removethis, this));
